...
import {
  ...
  useAnswer,
  useSetAnswer,
} from 'modules/components/Game/table-context/AnswersContext'
...
export const TableRow = ({
  ...
  useEffect(() => {
    const activeAnswer: TRow = answers.answers.filter(
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant="inherit" fontSize="1.5vw" fontWeight="bold">
          {symbol}
        </Typography>
      </Stack>
          ...
          >
            {gameStatus === 'FINISHED' && status === 'DONE' && (
              ...
              >
                <Typography variant="inherit">
                  {defaultValues && defaultValues.tableRowData[item.name]
                    ? defaultValues.tableRowData[item.name].point
                    : 0}
                </Typography>
              </Stack>
            )}
            ...
            {status === 'ACTIVE' ? (
              <TextField
                ...
                value={
                  answer[item.name] &&
                  answer[item.name].value.toString().length > 0
                ...
                disabled={
                  (!answer[item.name] ||
                    answer[item.name].value.toString().length === 1 ||
                    answer[item.name].value.toString()[0] !== symbol) &&
                  gameStatus === 'COUNTING'
                }
                InputProps={{
              ...
              defaultValues.tableRowData[item.name] &&
              defaultValues.tableRowData[item.name].value.toString().length >
                1 ? (
